														...
														<table>
															<tr>
																<td class="col-md-6 col-lg-6 col-xs-6">
																		...
																		<div class="input-prepend input-group">
																		<input type="text" class="form-control" placeholder="例如：及时渔"
		...
		{{/if}}
						...
						<div class="col-xs-8 col-sm-8 col-lg-8">
							<ul class="list-unstyled">
								...
								<li>
									<p style="margin-bottom: 0px;">
										<label style="width:80px;display:inline-block;">本渠道添加：</label>{{html epaperPageCount}}
										<i class=""></i>
									</p>
								</li>
								<li>
						...
						"aoColumns" : [
								...
								},
								{
									...
									"data" : function(row, type, set) {
										...
										return "<button onclick='lookDataForThis(this)'  class='lookDataForThis btn btn-xs btn-default' news_group_rule_id='"+id+"' rule_id='"+row.ruleId+"' group_id='"+row.groupId+"' title='查看数据'><i class='fa fa-search'></i></button>&nbsp&nbsp"
...
function lookDataForThis(ele){
	...
	$('#news-search-boxs').load("/epaper/epaperArticleSearch?eq_ruleId="+rule_id,function(){
		 var dialog = window.top.art.dialog({
				title: "详情",
...
function searchResult(){
	...
	var url = "/epaperGroupRule/getNewsGroupRuleList?l_paper="+section+"&eq_groupId="+groupId;
...
function initFailureRecordTableButton(){
	...
	var search_btn = '<button type="button" class="btn btn-default margin_R5" onclick="searchRequirement()">查 询</button>';
...
function searchRequirement(){
	var url = "/epaperGroupRule/getEpaperGroupRuleRequirementList";
	var eq_state = $("select[name='eq_state']").val();
	var l_section = $("input[name='l_section']").val();
	var l_userName = $("input[name='l_userName']").val();
	var params = {"eq_state":eq_state,"l_paper":l_section,"l_userName":l_userName};
		    ...
		    for(var i = 0 ; i < data.length;i++){
		    	...
		    	if(id){
		    		...
		    		strs+='  <div><b>标题</b>:<a target="blank_" href='+sectionUrl+'>'+section+'</a></div>'+
					    ...
					    ' 	<a class="btn btn-primary btn-xs" to-url="'+sectionUrl+'" href="javascript:void(0);" onclick="testListPageInfo(this)" style="display: block;width: 127px;margin:10px 0;">测试报刊列表</a>'+
...
function toRequirement(userId,userName){
	var url = "/epaperGroupRule/getEpaperGroupRuleRequirementList";
	var params = {"eq_state":"0","eq_userId":userId};
	url = addQueryParams(url,params);
	news_group_rule_requirement_list_table.fnSettings().sAjaxSource=url;
	news_group_rule_requirement_list_table.fnDraw(); //重新加载数据
	$('a[data-toggle="tab"]').trigger("click");
	$("select[name=eq_state]").val(0);
