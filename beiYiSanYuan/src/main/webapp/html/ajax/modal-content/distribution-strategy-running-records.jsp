			...
			<div class="jarviswidget jarviswidget-color-darken" style="margin:0px;" data-widget-editbutton="false" data-widget-colorbutton="false">
				...
				<div style="padding-top:10px;">
					...
					<div class="widget-body no-padding">
						...
						<table id="running_record_list" class="table table-striped table-bordered table-hover" width="100%">
							<thead>
								<tr>
     								<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 开始时间</th>
     								<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 推送文章数</th>
...
<script type="text/javascript">
	...
	var running_records_data_table ;
	// pagefunction	
	...
	function loadDataTable(){
		/* BASIC ;*/
		if(running_records_data_table!=null){
	        running_records_data_table.fnDestroy();
		}
		running_records_data_table = $("#running_record_list").dataTable({
			"bProcessing" : true,//加载数据时候是否显示进度条
			"bServerSide" : true,//是否从服务加载数据 
			"bSort":false,
			"sAjaxSource" : "/distribution-record/getRuningRecordAll?orderBy=time-desc&eq_distributionStrategyId=${distributionStrategyId}",//如果从服务器端加载数据,这个属性用于指定加载的路径
			"columns": [//数据列
	            {"data": function ( row, type, set ){
		            return row.jobStartTime;
	            }},
	            {"data": function ( row, type, set ){
	            	if(row.totalArticles>0){
	            		return "<a href='javascript:;' onclick=\"lookPushedArticles('"+row.id+"')\">"+row.totalArticles+"</a>";
	            	}
			...
			"fnInitComplete": function(oSettings, json) {
				$("#running_record_list_wrapper .dt-toolbar").remove();
		     }
	    });
	}
		...
		$.ajax({
				...
				success:function(data){
					if(data.result==1){
						...
						running_records_data_table.fnDraw(); //重新加载数据
					}else{
	...
	function lookPushedArticles(id){
		var buttons = [];
		callDialog("/distribution-record/lookPushedArticles?distributionRecordId="+id,"pushedArticles","已推送的文章","700",buttons);
	}
