...
<style>
	...
	.ColVis{display:none;}
	.tooltip{opacity:1;min-width:400px;background:none;}
	td:nth-child(8) .tooltip-inner{min-width:388px;}
			...
			<div class="jarviswidget" id="wid-id-1" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-colorbutton="false" data-widget-custombutton="false">
				...
				<div>
					...
					<div class="widget-body no-padding">
						...
						<form action="" class="smart-form" method="post" action="bdzd/getQuestionPagedList">
							...
							<fieldset>
								...
								<section class="col col-2">
									<label class="label">主站</label>
										<label class="input">
											<input type="text" class="input-sm " name="siteName">
										</label>
								</section>
								
								
								<section class="col col-2">
									<label class="label">提问人</label>
										<label class="input">
											<input type="text" class="input-sm" name="askPeople">
										</label>
						...
						<table id="site_list" class="table table-striped table-bordered table-hover" width="100%">
							<thead>
								<tr>
									...
									<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 类型</th>
									<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 提问时间</th>
...
<script type="text/javascript">
	...
	$(function(){
		    ...
		    $("#sub_button").click(function(){
		    	var url = "/bdzd/getQuestionPagedList";
		    	var title = $("input[name='title']").val();
		    	var site = $("input[name='siteName']").val();
		    	var askPeople = $("input[name='askPeople']").val();
		    	var s_time = $("input[name='s_time']").val();
		    	var e_time = $("input[name='e_time']").val();
		    	var params = {"l_title":title,"l_siteName":site,"l_askPeople":askPeople,
	...
	//var tableDestroyer = [];
	function showAnswers(obj,id){
		$("#dialog_edit").load("/bdzd/editQuestion?id="+id+"&showAnswersFlag=1");
		$('#dialog_edit').dialog("open");
	}
	function openDialog(obj,id)
	{
		$("#dialog_edit").load("/bdzd/showAnswers?id="+id);
		$('#dialog_edit').dialog("open");
		return false;
	}
	function deleteRow(id){
		$("#dialog_delete").load("/bdzd/toDelete?id="+id);
	...
	var pagefunction = function() {
		...
		*/	
		...
		<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 标题</th>
									...
									<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 提问人</th>
									<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 图片</th>
									<th><i class="fa fa-fw fa-user text-muted hidden-md hidden-sm hidden-xs"></i> 额外字段</th>
		*/
		data_table = $("#site_list").dataTable({
			...
			"sAjaxSource" : "/bdzd/getQuestionPagedList",//如果从服务器端加载数据,这个属性用于指定加载的路径 
			"columns": [//数据列
	            ...
	            {"data": function ( row, type, set ){
	            	if(!row.siteName){return "无";}
	            	return "<a href='"+row.siteUrl+"' target= '_blank'>"+row.siteName+"</a>";
	            },"bSortable": false},
	            {"data": function ( row, type, set ){
	            ...
	            {"data": function ( row, type, set ){
	            ...
	            },"bSortable": false},
	             {
		            ...
		            "sClass": "center",
		             "bSortable": false,
		            "data": function ( row, type, set ){
		            //"<button  onclick=\"openDialog(this,'"+row.id+"');\" title=\"编辑\" class=\"btn btn-xs btn-default\"><i class=\"fa fa-pencil\"></i></button>"
				        return "" 
				        +"<button  onclick=\"showAnswers(this,'"+row.id+"');\" title=\"显示答案\" class=\"btn btn-xs btn-default\"><i class=\"fa fa-pencil\"></i></button>"
