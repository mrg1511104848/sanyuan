...
// pagefunction	
	var pagefunction = function() {
		...
		data_table = $("#news_articles_list").dataTable({
			...
			"sAjaxSource" : "/weibo/getWeiboPagedList?orderBy=time-desc",//如果从服务器端加载数据,这个属性用于指定加载的路径 
			"columns": [//数据列
				...
				{"sWidth" : "10%","data": function( row, type, set ){
	            	var weiboUserName = row.weiboUserName;
			...
			"fnInitComplete": function(oSettings, json) {
				var ipt_weiboUserName = '<div class="btn-group margin_R5"><input type="text" class="form-control " name="weiboUserName" placeholder="用户名"></div>';
				...
				$("#sub_button").click(function(){
			    	var url = "/weibo/getWeiboPagedList?orderBy=time-desc";
			    	var weiboUserName = $("input[name='weiboUserName']").val();
			    	var topic = $("input[name='topic']").val();
			    	var params = {"l_weiboUserName":weiboUserName,"l_topic":topic};
	...
	function delAllArticle(){
		...
		$.ajax({
			url:'/weibo/delete.html?ids='+del_article_ids,
			success:function(d){
				if(d){
	...
	{
		loadingShow();
		$("#dialog_edit").load("/weibo/edit?eq_id="+id,function(){
