						...
						<div class="tab-content">
							<div class="tab-pane active" id="hr1">
								<div id="inbox-content" class="row" style="min-height: 520px;padding:0 10px;">
									<div class="tree smart-form col-sm-12 col-md-12 col-lg-2 group_tree" style="height: 500px;overflow-y: auto;display:none;"  id="admin_oprate_box">
									</div>
									
										<div style="overflow-y:auto;max-height: 510px;display:none;" id="user_oprate_box" class="col-sm-12 col-md-12 col-lg-2">
											<h6>
											<span style="padding-left:5px;display:inline-block;">所有任务</span>
											<c:if test="${currentUser.isAdmin!=1}">
											<a href="javascript:deleteGroup();" rel="tooltip" title="" data-placement="left"
														data-original-title="删除" class="pull-right txt-color-darken margin_LR5">
												<i class="fa fa-trash-o"></i>
											</a>
											<a href="javascript:addGroup(1);" rel="tooltip" title="" data-placement="left"
														data-original-title="修改" class="pull-right txt-color-darken margin_LR5">
												<i class="fa fa-pencil "></i>
											</a>
											<a href="javascript:addGroup(-1);" rel="tooltip" title="" 
													data-placement="left" data-original-title="添加" class="pull-right txt-color-darken margin_LR5" >
										...
										<div style="" id="auto_box " class="col-sm-12 col-md-12 col-lg-10">
...
function getAllGroupList(){
	$.ajax({
	   	...
	   	success:function(data){
	   		...
	   		$("#ul_group").html(htm);
	   		$("#span_group_id").html("&nbsp;&nbsp;&nbsp;任务ID（groupId）:"+$("#ul_group .active a").attr("site_id"));
	   	}
	});
}
function getAllTreeGroupList(){
	$.ajax({
	   	url:"news/getTreeGroupList.html",
	   	dataType:"json",
	   	success:function(data){
	   		var htm="<ul>";
	   		htm+="<li >";
	   		htm+="<span><i class='fa fa-lg fa-plus-circle' data-action='expand'></i> 用户</span>";
	   		htm+="<ul >";
	   		$.each(data,function(i,n){
	   			htm+="<li >";
	   			htm+='<span><i class="fa fa-lg fa-plus-circle" data-action="expand"></i> '+n.name+'</span>';
	   			htm+='<ul style="">';
	   				$.each(n.childGroups,function(i2,n2){
	   					htm+='<li style="display:none;"><span site_id="'+n2.id+'" onclick="selectGroup(\''+n2.id+'\',this)"><i class="icon-leaf"></i>'+n2.name+'</span><ul></ul></li>';
	   				});
	   			htm+='</ul>';
	   			htm+="</li >";
	   		});
	   		...
	   		$(".tree").find("li:has(ul)").addClass("parent_li").attr("role","treeitem").find(" > span").attr("title","Collapse this branch").on("click",
	   		function(a){
	   		...
	   		//loadScript("/resources/js/plugin/bootstraptree/bootstrap-tree.min.js");
	   	}
	});
}
function selectGroup(id,obj)
{
	...
	var isChecked =$("#all_sites").is(":checked");
	if(isChecked){
	...
	var group_id = $(obj).attr("site_id");
	if(group_id){
		$(".tree li span").css("border","1px dotted #999").css("backgroundColor","white").css("color","black");
		$(obj).css("backgroundColor","#3276b1").css("border","1px solid #3276b1").css("color","white");
		$("#span_group_id").html("&nbsp;&nbsp;&nbsp;groupId:"+group_id);
	}else{
		$("#span_group_id").html("&nbsp;&nbsp;&nbsp;groupId:"+$("#ul_group .active a").attr("site_id"));
	}
}
...
function InitList() {
	...
	$(".tools-bar").css("float","left").append(tools_html);	
	if(isAdmin!=1){
	$(".tools-bar").next().css("text-align","right").css("float","right").append(buttons);
	}	
	...
	$("#news_group_rule_list_wrapper .dt-toolbar").css("padding","0px 0px 5px");
		...
		if($("#ul_group .active").find("a").length>0)
	    {
	        ...
	        $.ajax( {
				...
				success : function(result) {
						if($("#news_group_rule_list #all_sites").is(':checked'))
						{
							$("#news_group_rule_list #all_sites").removeAttr("checked");
						}
						//oTable.fnClearTable();
						//aaaa
						oTable.fnDraw();
...
function addOrCancelSiteRules(ele,site_id){
	...
	if(html_=="添加"){
		...
		var group_name = $("#ul_group li.active a").text();
