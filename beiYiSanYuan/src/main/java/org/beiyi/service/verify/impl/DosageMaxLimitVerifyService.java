...
//			}
			...
			for (InstructionUse instructionUse : instructionUses) {
				...
				if(instructionDoseSelection.matches("单日最大剂量/最大日剂量|单日最大剂量|最大日剂量")){
					//比较单日剂量上限
					DosageCheckResult dosageIsMatchDayLimit = dosageIsMatchDayLimit(standardDosageAndUnit, instructionDoseSelection, instructionDosingFrequency);
					if(!dosageIsMatchDayLimit.isValid()){
						dosageCheckRecord.setInValidText("超出最大日剂量");
						dosageCheckRecord.setInvalidDosageType(VerifyTypeEnums.INVALID_DOSAGE_GT_DAY_LIMIT);
			...
			if(dosageCheckRecords.size()>0){
				ChuFangCheckRecord chuFangCheckRecord = dosageCheckRecords.get(0);
				String cfDrugCombinationName = chuFangCheckRecord.getChuFangDrug().getDrugCombinationName();
				String cfDosage = chuFangCheckRecord.getChuFangDrug().getDosage();
				String cfDosageUnit = chuFangCheckRecord.getChuFangDrug().getDosageUnit();
				String doseSelection = chuFangCheckRecord.getInstructionUse().getDoseSelection();
				String insDosage = chuFangCheckRecord.getInstructionUse().getDosage();
				String insDosageUnit = chuFangCheckRecord.getInstructionUse().getDosageUnit();
				errMsgBuffer.append(String.format("药品 “%s” 剂量为 “%s%s” ，超出【%s】 “%s%s”", 
						cfDrugCombinationName,cfDosage,cfDosageUnit,doseSelection,insDosage,insDosageUnit));
			}else{
	...
	private DosageCheckResult dosageIsMatchDayLimit(String[] standardDosageAndUnit,String instructionDoseSelection,String instructionDosingFrequency){
		...
		if(instructionDoseSelection.matches("单日最大剂量/最大日剂量|单日最大剂量|最大日剂量")){
			Integer oneDayTime = DosingFrequencyUtil.parseDosingFrequency(instructionDosingFrequency);
			if(oneDayTime == null){
